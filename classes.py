...
class Database : 
    ...
    def retrieve_id(self, Ui_object) :
        ...
        for name in [Ui_object.product_chosen, Ui_object.substitute_name] : 
            #A stocker dans database.retrieve_data()
            self.cursor.execute("SELECT id FROM 5db.Produit WHERE nom = %s", [name]) 
    ...
    def add_to_table(self, table_name, values) : 
        ...
        "5db.Recherche" : " (product_id, substitute_id) VALUES (%s, %s)"} 
        ...
        try : 
            ...
            #If table == 5db_recherche : switch variable de verification à True

        except mysql.connector.errors.IntegrityError :
            if self.verbose == True :
                print("Item déjà enregistré dans {} !".format(table_name))
